...
testing_datalist = 'image4LSTM/test/test_depth_DF.txt'
...
load_params = tl.files.load_npz(name='depth_DF_model.npz')
...
for X_indices, y_label_t in tl.iterate.minibatches(X_teidx,
  ...
  print predict_value
  np.save('test_depth_DF_predict/' + image_path[0][-7:] + '.npy', predict_value)
