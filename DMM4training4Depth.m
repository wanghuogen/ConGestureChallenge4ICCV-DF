...
DMM_dirf = ['DFDI4caffe/train/DepthDMM/'];
if ~exist(DMM_dirf)
    mkdir(DMM_dirf);
end
...
foutid1 = fopen('DFDI4caffe/train/train_depth.txt','w');
foutid2 = fopen('DFDI4caffe/train/val_depth.txt','w');
...
for i = 1:length(name_list)
    ...
    D_name = [dataPathTrain name_list{i} '.K.avi'];
    ...
    for j=2:segNo
        ...
        classDir = strcat('DFDI4caffe/train/DepthDMM/',sprintf('%03s',num2str(tempClass-1)));
        if ~exist(classDir)
            mkdir(classDir);
        end
