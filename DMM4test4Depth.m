...
clc;
%% set path
dataPath = 'DATA/ConGD_phase_2/'
dataPathTrain=[dataPath 'test/'];
list_file_path = 'test_segmented.list';

segData=importdata(list_file_path);
[name_list] = textread(list_file_path, '%s %*[^\n]');

DMM_dir = ['DFDI4caffe/test/DMM4Depth/'];
if ~exist(DMM_dir)
    mkdir(DMM_dir);
end
baseFName = 0;
...
for i = 1:length(name_list)
    ...
    for j=2:segNo
        ...
        outImageName = fullfile(DMM_dir,sprintf('%05s.K.jpg',num2str(baseFName)));
